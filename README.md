...
Cascade 설정된 Entity 의 삭제 API에 대한 부하테스트 결과 및 해당 결과의 해석 및 리팩토링에 대해 작성된 문서입니다. 
...
연관 관계의 주인이 되는 Entity 의 삭제 과정은, 해당 Entity 를 외래 키로 갖는 모든 데이터의 삭제를 의미한다. 따라서 삭제 과정에서 연결되어있는
테이블의 수만큼의 삭제 쿼리가 발생하게 된다. 
