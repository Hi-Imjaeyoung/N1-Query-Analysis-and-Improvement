...
N+1 쿼리 튜닝에도 불구하고 p(95) 3.45초는 동기 방식 API로서 서비스 불가 수준이라 생각된다. 엑셀 생성이라는 "무거운 작업(Heavy Task)"은 @Async 또는 **메시지 큐(MQ)**를 도입해 비동기 방식으로 전환, API 응답 시간을 1초 미만으로 개선해야 함을 목표로 삼고 리팩토링을 진행할 예정이다. 
